...
export class ArenaMode {
    ...
    constructor(private naubino: Naubino) {
        ...
        this.arena_body = Matter.Bodies.circle(this.arena.x, this.arena.y, this.arena.radius, {
            label: 'arena.sensor',
            isSensor: true,
        });
        Matter.World.add(this.naubino.engine.world, this.arena_body);
        naubino.collider.ee.on(this.arena_body.label, (label_a, label_b, pair) =>
            this.on_arena_sensor(label_a, label_b, pair)
        );
    }
